...
const DashboardsDefault = () => {
  ...
  useEffect(() => {
    allProfiles(1, 1000).then((res) => {
      const c = res?.filter((el) => el?.type === 'customer');
    ...
    <>
            ...
            <select className='form-control form-select' name="room-type" id="room-type" onChange={(e) => handleSearch(e?.target?.value,"type")}>
              <option value="">Select the room type</option>
              {
                roomTypes?.map((rt, index) => {
                  return <option key={index} value={rt?.name}>{rt?.name}</option>
                })
              }
            </select>
          </Col>
        ...
        <Row className='mb-4'>
          {
            searchResults?.map((rm,index) => {
              return <Col key={index} md={4} className='mb-4 mb-md-2'>
                {/* eslint-disable-next-line no-underscore-dangle */}
                <RoomItem selected={bookingData?.room?.includes(rm?.item?._id)} onSelected={() => handleSelectRoom(rm?.item?._id)} name={rm?.item?.name} height={250} price={rm?.item?.bonusPrice} src={rm?.item?.images?.[0]} />
              </Col>
            })
          }
        </Row>
      ...
      { step ===2 && <>
        <Row className="mb-md-4">
          ...
          <Col md={4} className='mb-4 mb-md-0'>
            ...
            {/* eslint-disable-next-line jsx-a11y/no-onchange */}
            <select disabled={customerType === 'new'} onChange={(e) => handleSelectCustomer(e?.target?.value)} className='form-control form-select' name="room-type" id="room-type">
              <option value="">Select the customer</option>
              {
                customers?.map((cus, index) => {
                  // eslint-disable-next-line no-underscore-dangle
                  return <option key={index} value={cus?._id}>{`${cus?.firstName} ${cus?.otherNames} ${cus?.lastName}`}</option>
                })
              }
            </select>
          </Col>
      ...
      { step ===3 && <>
        <Row className="mb-md-4">
          ...
          <Col md={4} className='mb-4 mb-md-0'>
            <label htmlFor="amount">Paid <span className='text-danger font-weight-bold'>*</span></label>
            <input name="paid" value={bookingData?.paid} type="text" className="form-control" id="amount" onChange={handleBookingDataChange}/>
          </Col>
          <Col md={4} className='mb-4 mb-md-0'>
            <label htmlFor="balance">Balance <span className='text-danger font-weight-bold'>*</span></label>
            <input name="balance" value={bookingData?.balance} type="text" disabled placeholder='Balance' className="form-control" id="balance" onChange={handleBookingDataChange}/>
          </Col>
        </Row>
        <Row className="mb-4">
          <Col className="mb-4">
